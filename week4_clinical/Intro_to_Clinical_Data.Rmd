...
```{r}
write.csv(clinic, "/Users/alexvillachica/desktop/QBIO490/qbio_490_Alexisvillachica/analysis_data/skcm_clinical_data.csv", row.names=FALSE)
```

To read the csv back into your environment as a dataframe:

```{r}
clinic <- read.csv("/Users/alexvillachica/desktop/QBIO490/qbio_490_Alexisvillachica/analysis_data/skcm_clinical_data.csv")
```
...
# Should use the str() to see the structure and head() to see the first few lines
...
#3
(as.numeric(clinic[5,"birth_days_to"]) * -1) / 365

#4
((as.numeric(clinic[2,"birth_days_to"]) * -1) - (as.numeric(clinic[2,"last_contact_days_to"]))) / 365
...
```{r Exercise 4.1}
plot(clinic$age_at_diagnosis, clinic$tumor_stage, xlab = "Age at Diagnosis", ylab = "Tumor Stage", main = "Age at Diagnosis vs Tumor Stage", col = "blue")
```
...
boxplot(clinic$age_at_diagnosis ~ clinic$vital_status, data = clinic, xlab = "Gender", ylab = "Age at Diagnosis", main = "Age at Diagnosis by Gender", col = "brown")
```
...
```{r}
male_mask <- ifelse(clinical$gender == 'MALE', TRUE, FALSE)
```
...
```{r}
cleaned_clinic$age_category <- ifelse(cleaned_clinic$age_at_diagnosis <= 35, "Young", ifelse(cleaned_clinic$age_at_diagnosis >= 65, "Old", "Middle"))
```
...
survival_time_mask <- is.na(cleaned_clinic$survival_time)
cleaned_clinic <- cleaned_clinic[!survival_time_mask, ]
```
...
```{r}
survival_object <- Surv(time = cleaned_clinic$survival_time, event = cleaned_clinic$death_event)
...
The y axis represents the survival probability of a middle, old, or young patient.

# 2. What does the x axis represent?
The x axis represents the amount of time in days a typical skin cancer patients survives when diagnosed with skin cancer.

# 3. What age group appears to have the greatest rate of survival?
Young patients appear to have the greatest rate of survival.

# 4. What age group appears to have the worst rate of survival? 
Old patients appear to have the worst rate of survival.

# 5. What is the approx. 15-year survival rate for "Old" skin cancer patients?
The approximate 15 year survival rate for "Old" skin cancer patients looks to be around 0.00% to ~18.00% roughly.
...
The p value is less than 0.0001. The reason the p-value is so statistically significant is because it reveals the strength in evidence from the results. It tells us age diagnosed is strongly correlated with the chance of survival for a skin cancer patient.
